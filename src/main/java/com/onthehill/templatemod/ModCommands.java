 ...
 * {@code /template-mod-admin}. Every command here is op-gated per the studio
...
public final class ModCommands
{
    ...
    /**
     * Registers the {@code /template-mod-admin} command tree.
     *
        ...
        java.util.function.BiConsumer<ServerPlayer, ServerProgressConfig> onValidUpdateApplied)
    {
        ...
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) ->
            dispatcher.register(Commands.literal(TemplateMod.MOD_ID + "-admin")
                ...
                .then(Commands.literal("config")
                        ...
                        .then(Commands.argument("value", FloatArgumentType.floatArg())
                            .executes(context ->
                            {
                                ...
                                context.getSource().sendSuccess(
                                    () -> Component.literal("template-mod progress rate set to " + proposed), true);
                                return 1;
                            })))
                    .then(Commands.literal("allow-read-only")
                        .then(Commands.argument("value", BoolArgumentType.bool())
                            .executes(context ->
                            {
                                ...
                                context.getSource().sendSuccess(
                                    () -> Component.literal("template-mod non-op read-only view set to " + proposed), true);
                                return 1;
                            })))
                    .then(Commands.literal("reset")
                        .executes(context ->
                        {
                            ...
                            context.getSource().sendSuccess(
                                () -> Component.literal("template-mod server config reset to defaults"), true);
                            return 1;
                        })))));
    }
}
