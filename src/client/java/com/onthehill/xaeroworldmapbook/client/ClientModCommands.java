 ...
 * {@code /xaero-world-map-book}, handled entirely locally via
 * {@code ClientCommandRegistrationCallback}. This root and
 * {@code /xaero-world-map-book-admin} (see {@code ModCommands}) deliberately share
 * no literal beyond the bare mod-id prefix, and nothing under
 * {@code /xaero-world-map-book-admin} is ever registered here — see the studio
...
public final class ClientModCommands
{
    ...
    /**
     * Registers the {@code /xaero-world-map-book} command tree.
     *
                ...
                .then(ClientCommands.literal("config")
                        ...
                        .then(ClientCommands.argument("mode", StringArgumentType.word())
                            ...
                            .executes(context ->
                            {
                                ...
                                context.getSource().sendFeedback(
                                    Component.literal("xaero-world-map-book visualization set to " + raw));
                                return 1;
                            })))
                    .then(ClientCommands.literal("reset")
                        .executes(context ->
                        {
                            ...
                            config.save(clientConfigPath());
                            context.getSource().sendFeedback(
                                Component.literal("xaero-world-map-book client config reset to defaults"));
                            return 1;
                        })))));
    }
    ...
    {
        return FabricLoader.getInstance().getConfigDir().resolve(XaeroWorldMapBook.MOD_ID).resolve("client.json");
    }
}
