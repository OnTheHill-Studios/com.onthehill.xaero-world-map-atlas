...
public final class SaveConfirmScreen extends Screen
{
    ...
    {
        super(Component.translatable("gui.template-mod.confirm_save.title"));
    ...
    protected void init()
    {
        ...
        MultiLineTextWidget messageWidget = new MultiLineTextWidget(
            Component.translatable("gui.template-mod.confirm_save.message"), this.font);
        messageWidget.setMaxWidth(MESSAGE_MAX_WIDTH);
        messageWidget.setCentered(true);
        content.addChild(messageWidget);

        content.addChild(Button.builder(
            Component.translatable("gui.template-mod.confirm_save.save_and_close"),
            button -> this.onSaveAndClose.run()
        ).bounds(0, 0, BUTTON_WIDTH, BUTTON_HEIGHT)
            .tooltip(Tooltip.create(Component.translatable("gui.template-mod.confirm_save.save_and_close_tooltip")))
            .build());

        content.addChild(Button.builder(
            Component.translatable("gui.template-mod.confirm_save.close_without_saving"),
            button -> this.onCloseWithoutSaving.run()
        ).bounds(0, 0, BUTTON_WIDTH, BUTTON_HEIGHT)
            .tooltip(Tooltip.create(Component.translatable("gui.template-mod.confirm_save.close_without_saving_tooltip")))
            .build());

        content.addChild(Button.builder(
            Component.translatable("gui.template-mod.confirm_save.cancel"),
            button -> this.onCancel.run()
        ).bounds(0, 0, BUTTON_WIDTH, BUTTON_HEIGHT)
            .tooltip(Tooltip.create(Component.translatable("gui.template-mod.confirm_save.cancel_tooltip")))
            .build());
