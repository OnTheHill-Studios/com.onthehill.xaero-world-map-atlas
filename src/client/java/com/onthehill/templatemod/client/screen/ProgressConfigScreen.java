...
import com.onthehill.templatemod.client.ClientModCommands;
import com.onthehill.templatemod.client.TemplateModClient;
import com.onthehill.templatemod.client.config.ClientVisualizationConfig;
import com.onthehill.templatemod.client.config.ProgressColorUtil;
...
import com.onthehill.templatemod.network.AdminConfigUpdatePayload;
 ...
 * command opens it on each tab ({@code /template-mod gui} → client,
 * {@code /template-mod-admin gui} → admin via {@code OpenAdminScreenPayload}),
...
public final class ProgressConfigScreen extends Screen
{
    ...
    {
        super(Component.translatable("gui.template-mod.config.title"));
        this.parent = parent;
        this.activeTab = initialTab;

        // The shared instance TemplateModClient/ProgressHudRenderer read
        ...
        this.clientConfig = TemplateModClient.getClientConfig();
    ...
    /**
     * Opens the screen on the client tab. Used by {@code /template-mod gui}
    ...
    private void buildTabRow()
    {
        ...
        addRenderableWidget(Button.builder(
            Component.translatable("gui.template-mod.config.tab_client"),
            button -> switchTab(Tab.CLIENT)
        ).bounds(left, 8, TAB_BUTTON_WIDTH, ROW_HEIGHT)
            .tooltip(Tooltip.create(Component.translatable("gui.template-mod.config.tab_client_tooltip")))
            .build());

        addRenderableWidget(Button.builder(
            Component.translatable("gui.template-mod.config.tab_admin"),
            button -> switchTab(Tab.ADMIN)
        ).bounds(left + TAB_BUTTON_WIDTH, 8, TAB_BUTTON_WIDTH, ROW_HEIGHT)
            .tooltip(Tooltip.create(Component.translatable("gui.template-mod.config.tab_admin_tooltip")))
            .build());
    }
    ...
    private void buildBottomRow()
    {
        ...
        addRenderableWidget(Button.builder(Component.translatable("gui.template-mod.config.save"), button -> save())
            .bounds(left, y, BOTTOM_BUTTON_WIDTH, ROW_HEIGHT)
            .tooltip(Tooltip.create(Component.translatable("gui.template-mod.config.save_tooltip")))
            .build());

        addRenderableWidget(Button.builder(Component.translatable("gui.template-mod.config.close"), button -> onClose())
            .bounds(left + BOTTOM_BUTTON_WIDTH + BOTTOM_BUTTON_SPACING, y, BOTTOM_BUTTON_WIDTH, ROW_HEIGHT)
            .tooltip(Tooltip.create(Component.translatable("gui.template-mod.config.close_tooltip")))
            .build());
    }
    ...
    private void buildClientTab(LinearLayout content)
    {
        CycleButton<ProgressVisualizationMode> visualizationControl = CycleButton.builder(
            ...
            .create(0, 0, CYCLE_WIDTH, ROW_HEIGHT,
                Component.translatable("gui.template-mod.config.visualization_label"),
                (button, newValue) ->
                {
        ...
        visualizationControl.setTooltip(Tooltip.create(
            Component.translatable("gui.template-mod.config.visualization_tooltip")));
    ...
    private LinearLayout buildColorRow()
    {
        ...
        ProgressColorSwatchWidget swatch = new ProgressColorSwatchWidget(0, 0, COLOR_SWATCH_SIZE, COLOR_SWATCH_SIZE,
            ...
            openPicker);
        swatch.setTooltip(Tooltip.create(Component.translatable("gui.template-mod.config.visualization_color_tooltip")));
        row.addChild(swatch);

        Button colorButton = Button.builder(
            Component.translatable("gui.template-mod.config.visualization_color_label"),
            button -> openPicker.run()
        ).bounds(0, 0, COLOR_BUTTON_WIDTH, ROW_HEIGHT)
            .tooltip(Tooltip.create(Component.translatable("gui.template-mod.config.visualization_color_tooltip")))
            .build();
    ...
    private void buildAdminTab(LinearLayout content)
    {
        ...
        {
            centeredMessage(content, Component.translatable("gui.template-mod.config.admin_unavailable"));
            return;
        }

        if (!viewerIsOperator && !permitted)
        {
            centeredMessage(content, Component.translatable("gui.template-mod.config.admin_no_permission"));
            return;
        }

        if (!viewerIsOperator)
        {
            centeredMessage(content, Component.translatable("gui.template-mod.config.admin_read_only_note"));
        }
        ...
        CycleButton<Boolean> readOnlyToggle = CycleButton.onOffBuilder(this.pendingAllowNonOpReadOnlyView)
            .create(0, 0, CYCLE_WIDTH, ROW_HEIGHT,
                Component.translatable("gui.template-mod.config.allow_read_only_label"),
                (button, newValue) ->
                {
        ...
        readOnlyToggle.setTooltip(Tooltip.create(
            Component.translatable("gui.template-mod.config.allow_read_only_tooltip")));
    ...
    private LinearLayout buildRateRow(boolean interactive)
    {
        ...
        StringWidget label = new StringWidget(RATE_LABEL_WIDTH, ROW_HEIGHT,
            Component.translatable("gui.template-mod.config.progress_rate_label"), this.font);
        label.setTooltip(Tooltip.create(
            Component.translatable("gui.template-mod.config.progress_rate_label_tooltip")));
        row.addChild(label);
        ...
        value.setTooltip(Tooltip.create(
            Component.translatable("gui.template-mod.config.progress_rate_value_tooltip")));
        row.addChild(value);

        Button decreaseButton = Button.builder(
            Component.translatable("gui.template-mod.config.decrease"),
            ...
            .tooltip(Tooltip.create(Component.translatable("gui.template-mod.config.decrease_tooltip")))
            .build();
        Button increaseButton = Button.builder(
            Component.translatable("gui.template-mod.config.increase"),
            button -> { this.pendingProgressRate = Math.min(ServerProgressConfig.MAX_PROGRESS_RATE, this.pendingProgressRate + 0.005f); rebuild(); }
        ).bounds(0, 0, RATE_STEP_BUTTON_WIDTH, ROW_HEIGHT)
            .tooltip(Tooltip.create(Component.translatable("gui.template-mod.config.increase_tooltip")))
            .build();
    ...
    private LinearLayout sectionResetRow(Runnable resetAction, boolean interactive)
    {
        ...
        ProgressConfigResetButton reset = ProgressConfigResetButton.create(0, 0, RESET_WIDTH,
            Component.translatable("gui.template-mod.config.reset_section"), button -> resetAction.run());
    ...
    {
        ProgressConfigResetButton reset = ProgressConfigResetButton.create(0, 0, RESET_WIDTH,
            Component.translatable("gui.template-mod.config.reset_field"), button -> onPress.run());
        reset.active = interactive;
        return reset;
    }
    ...
    private static Component visualizationLabel(ProgressVisualizationMode mode)
    {
        return mode == ProgressVisualizationMode.RADIAL
            ? Component.translatable("gui.template-mod.config.visualization_radial")
            : Component.translatable("gui.template-mod.config.visualization_bar");
    }
